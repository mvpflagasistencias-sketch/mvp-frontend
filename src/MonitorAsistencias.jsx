...
const MonitorAsistencias = ({ onBack }) => {
  ...
  const [fotoModalTitulo, setFotoModalTitulo] = useState(
      ...
      if (isModalOpen && asistenciaSeleccionada?.id_jugador) {
        try {
          const res = await api.get(
            `/api/jugadores/${asistenciaSeleccionada.id_jugador}/asistencias-acumuladas`,
          );
    ...
    <div className="space-y-6 animate-in fade-in duration-500 text-left relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-[#141b2e] p-4 rounded-2xl border border-gray-800">
        {(activeTab === "partidos" || activeTab === "historial") && (
          <div>
            <label className="text-[10px] text-gray-500 font-black uppercase block mb-1">
              Filtrar Jornada
            </label>
            <select
              value={filtroJornada}
        ...
        <div className="grid grid-cols-1 gap-4">
            ...
            .map((p) => {
              ...
              return (
                ...
                >
                  <div
                    onClick={() =>
                      setPartidoExpandido(isSelected ? null : matchKey)
                    }
                  ...
                  >
                    <div>
                      ...
                      <h3 className="text-xl font-extrabold text-white uppercase tracking-tight">
                        {p.equipo_local}{" "}
                        <span className="text-green-500 font-black text-sm lowercase px-1">
                          vs
                        </span>{" "}
                        {p.equipo_visitante}
                      </h3>
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
                      ...
                      <div className="flex gap-1.5 items-center bg-[#0f172a]/80 p-1.5 rounded-xl border border-gray-800">
                        {p.foto_local_url ? (
                          <div
                            className="w-9 h-9 bg-gray-900 border border-blue-500/40 rounded-lg overflow-hidden flex items-center justify-center relative"
                            title="Planilla Local"
                          >
                            <img
                            ...
                            />
                            <span className="absolute bottom-0 inset-x-0 bg-blue-900/90 text-[7px] text-center text-white font-black uppercase scale-75">
                              L
                            </span>
                          </div>
                        ) : (
                          <div className="w-9 h-9 bg-gray-900/40 border border-dashed border-gray-800 rounded-lg flex items-center justify-center text-[8px] text-gray-700 font-bold">
                            L
                          </div>
                        )}
                        ...
                        {p.foto_visitante_url ? (
                          <div
                            className="w-9 h-9 bg-gray-900 border border-red-500/40 rounded-lg overflow-hidden flex items-center justify-center relative"
                            title="Planilla Visitante"
                          >
                            <img
                              src={p.foto_visitante_url}
                              alt="Visitante"
                              className="w-full h-full object-cover"
                            />
                            <span className="absolute bottom-0 inset-x-0 bg-red-900/90 text-[7px] text-center text-white font-black uppercase scale-75">
                              V
                            </span>
                          </div>
                        ) : (
                          ...
                          <div className="p-4 bg-[#141b2e] border border-gray-800 rounded-2xl flex items-center justify-between gap-4">
                            <div>
                              <p className="text-blue-400 text-xs font-black uppercase tracking-wider">
                                📸 Planilla Roster Local
                              </p>
                              <p className="text-gray-500 text-[9px] uppercase truncate max-w-[180px] sm:max-w-xs">
                                {p.equipo_local}
                              </p>
                            </div>
                            {p.foto_local_url ? (
                              ...
                              <span className="text-gray-600 font-bold text-[10px] uppercase italic">
                                Sin Captura
                              </span>
                            )}
                          </div>
                            ...
                            {p.foto_visitante_url ? (
                              <button
                                onClick={() =>
                                  abrirVisorEvidencia(
                                    p.foto_visitante_url,
                                    `Planilla Visitante - ${p.equipo_visitante}`,
                                  )
                                }
          ...
          {partidosAgrupados.length === 0 && (
            <div className="bg-[#1e293b] p-12 text-center rounded-3xl border border-gray-800 text-gray-500 italic">
              No hay partidos registrados bajo los criterios de filtrado
              seleccionados.
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[#1e293b] p-6 rounded-3xl border border-gray-800 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#0f172a] p-4 rounded-2xl border border-gray-800 text-left">
                <p className="text-gray-500 text-[9px] font-black uppercase tracking-wider">
                  Escaneos Totales
                </p>
          ...
          <div className="bg-[#1e293b] p-6 rounded-3xl border border-gray-800 space-y-3">
            ...
            <div className="space-y-2 text-xs text-gray-400 uppercase font-bold">
              <p>
                📌 Jornadas con Actividad Real:{" "}
                <span className="text-white font-mono">
                  {jornadasUnicas.length === 0 ? 0 : jornadasUnicas.join(", ")}
                </span>
              </p>
              <p>
                🏆 Categorías en Operación Móvil:{" "}
                <span className="text-white font-mono">
                  {categoriesUnicas.length === 0
                    ? "Esperando..."
                    : categoriesUnicas.join(", ")}
                </span>
              </p>
              <p>
                ⚡ Tasa de Cobertura de Geolocalización:{" "}
                <span className="text-green-400 font-mono">
                  {totalPasesRegistrados > 0
                    ? `${((pasesConGps / totalPasesRegistrados) * 100).toFixed(1)}%`
                    : "0%"}
                </span>
              </p>
            </div>
        ...
        >
          ...
          >
            <div className="bg-gradient-to-r from-[#0f172a] to-green-900 p-8 text-left">
              <p className="text-green-400 text-[10px] font-black uppercase tracking-widest mb-1 italic">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-white font-bold uppercase">
                    {asistenciaSeleccionada.nombre_equipo || "Independiente"}
                  </p>
                </div>
                <div>
                  <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-1">
                  ...
                  <p className="text-green-400 font-bold uppercase italic">
                    {asistenciaSeleccionada.staff}
                  </p>
                </div>
                <div>
                  <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-1">
                  ...
                  <p className="text-white font-mono">
                    {formatearFechaLimpia(asistenciaSeleccionada.fecha)}
                  </p>
                </div>
                <div>
                  <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-1">
        ...
        >
          ...
          >
            <div className="bg-[#0f172a] p-5 flex justify-between items-center border-b border-gray-800">
              <div>
                <p className="text-green-400 text-[9px] font-black uppercase tracking-widest italic">
                  Visor de Evidencia Real
                </p>
