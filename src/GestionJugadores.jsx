...
const GestionJugadores = ({ alRegistro }) => {
  ...
  const handleFotoUploadInline = async (e, jugadorId) => {
    ...
    reader.onloadend = async () => {
      try {
        await api.put(`/api/jugadores/${jugadorId}/foto`, {
  ...
  if (loading)
    return (
      <div className="text-center p-10 text-blue-400 animate-pulse font-bold uppercase tracking-widest">
        ...
        <div className="bg-[#0f172a] p-6 border-b border-gray-700 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-left w-full md:w-auto">
            <h3 className="text-xl font-black text-white uppercase tracking-tighter italic">
              Bitácora Digital de Atletas
            </h3>
            <p className="text-gray-500 text-xs font-bold uppercase tracking-widest">
              Gestión de Credenciales QR
            </p>
          </div>
          ...
          <div className="flex items-center gap-6 w-full md:w-auto justify-end">
            ...
            <span className="text-blue-400 font-mono text-sm hidden lg:block">
              VISTA: {jugadoresFiltrados.length}
            </span>
          </div>
        </div>
                  ...
                  >
                    ...
                    <td className="p-5 space-x-6 text-center">
                      <button
                        ...
                        className="text-blue-400 hover:scale-125 transition-transform text-lg"
                      >
                        ✏️
                      </button>
                      <button
                        onClick={() => handleDelete(j.id, j.nombre)}
                        className="text-red-500 hover:scale-125 transition-transform text-lg"
                      >
                        🗑️
                      </button>
                    </td>
          ...
          >
            <div className="bg-gradient-to-r from-[#0f172a] to-blue-900 p-10 text-left relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div className="space-y-2">
                ...
                <p className="text-blue-300 font-mono text-xs uppercase">
                  Licencia Digital MVP FLAG
                </p>
              </div>
              ...
              {jugadorSeleccionado.foto_perfil && (
                <div className="w-20 h-20 bg-gray-900 rounded-2xl border-2 border-blue-400 overflow-hidden shadow-xl shrink-0 hidden sm:block">
                  <img
                    src={jugadorSeleccionado.foto_perfil}
                    alt="Rostro Registro"
            ...
            <div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="space-y-6 text-left flex flex-col justify-between">
                <div className="flex items-center gap-4 bg-[#141b2e] p-4 rounded-2xl border border-gray-800 relative group">
                  <label
                    className="w-20 h-20 bg-[#0f172a] rounded-xl border border-dashed border-gray-700 flex items-center justify-center overflow-hidden cursor-pointer shrink-0 hover:border-blue-500 transition-colors"
                  ...
                  >
                    {jugadorSeleccionado.foto_perfil ? (
                      ...
                      <span className="text-[8px] text-gray-600 font-black text-center uppercase p-1">
                        Cargar Foto
                      </span>
                    )}
                    <input
                      ...
                      className="hidden"
                      onChange={(e) =>
                        handleFotoUploadInline(e, jugadorSeleccionado.id)
                      }
                    />
                  </label>
                  <div className="min-w-0 flex-1">
                    <p className="text-white font-extrabold text-sm uppercase truncate">
                      Fotografía Oficial
                    </p>
                    <p className="text-gray-500 text-[9px] uppercase leading-relaxed">
                      Clic sobre el recuadro para actualizar la captura
                      biométrica del roster
                    </p>
                  </div>
                </div>

                <div className="flex-1 pt-2">
                  <h4 className="text-blue-500 text-[10px] font-black uppercase tracking-widest mb-3 italic underline decoration-blue-500/30">
                    Expediente
                  </h4>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-white font-mono">
                          {jugadorSeleccionado.telefono || "Sin Teléfono"}
                        </p>
                      </div>
                    </div>
                    {jugadorSeleccionado.tutor &&
                      jugadorSeleccionado.tutor.trim() !== "" && (
                        <div className="flex items-center gap-4">
            ...
            <div className="bg-[#0f172a]/50 p-8 flex gap-4">
              <button
                onClick={() => setIsViewModalOpen(false)}
                className="flex-1 bg-gray-800 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-gray-700 transition-all"
              >
                Cerrar
              </button>
              <button
                onClick={() => window.print()}
                className="flex-1 bg-cyan-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-cyan-500 transition-all shadow-lg shadow-cyan-900/30"
              >
                Imprimir Credencial
              </button>
            </div>
