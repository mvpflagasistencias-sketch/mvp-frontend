    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in zoom-in duration-500">
      ...
      <div className="flex justify-between items-end">
        <div className="text-left">
          <h2 className="text-3xl font-black text-white uppercase tracking-tighter italic">Panel de Control Staff</h2>
          <p className="text-orange-400 font-medium text-xs uppercase tracking-widest">Gestión de Accesos y Seguridad</p>
        </div>
        <button onClick={onBack} className="text-gray-400 hover:text-white transition-colors text-sm font-bold uppercase">
          ← Volver al Dashboard
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-[#1e293b] p-8 rounded-3xl border border-gray-700 shadow-2xl h-fit text-left">
          ...
          <form onSubmit={handleRegistro} className="space-y-4">
            <div>
              <label className="text-gray-500 text-[10px] font-black uppercase mb-1 block">Nombre del Staff</label>
              <input type="text" placeholder="Ej. Coach Carlos" className="w-full bg-[#0f172a] p-4 rounded-2xl border border-gray-700 text-white outline-none focus:border-orange-500 transition-all font-bold" onChange={e => setNuevoMiembro({...nuevoMiembro, nombre: e.target.value})} required value={nuevoMiembro.nombre} />
            </div>
            <div>
              <label className="text-gray-500 text-[10px] font-black uppercase mb-1 block">ID de Usuario</label>
              <input type="text" placeholder="UsuarioApp" className="w-full bg-[#0f172a] p-4 rounded-2xl border border-gray-700 text-white outline-none focus:border-orange-500 transition-all font-bold" onChange={e => setNuevoMiembro({...nuevoMiembro, usuario: e.target.value})} required value={nuevoMiembro.usuario}/>
            </div>
            <div>
              <label className="text-gray-500 text-[10px] font-black uppercase mb-1 block">Contraseña Provisional</label>
              <input type="password" placeholder="••••••••" className="w-full bg-[#0f172a] p-4 rounded-2xl border border-gray-700 text-white outline-none focus:border-orange-500 transition-all font-bold" onChange={e => setNuevoMiembro({...nuevoMiembro, password: e.target.value})} required value={nuevoMiembro.password}/>
            </div>
      ...
      {isViewModalOpen && staffSeleccionado && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-[#1e293b] w-full max-w-md rounded-3xl border border-orange-500/30 shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="bg-gradient-to-r from-[#0f172a] to-orange-900 p-8 text-left">
              <span className="bg-orange-500 text-[9px] px-2 py-1 rounded font-black uppercase tracking-widest text-white">Personal Autorizado</span>
              <h2 className="text-3xl font-black text-white uppercase mt-2 tracking-tighter italic leading-none">
                {staffSeleccionado.nombre}
              </h2>
              <p className="text-orange-300 font-mono text-sm mt-1">{staffSeleccionado.rol}</p>
            </div>
            ...
            <div className="p-8 space-y-6 flex flex-col items-center">
              ...
              <div className="grid grid-cols-2 gap-4 w-full">
                <div className="bg-[#0f172a] p-4 rounded-2xl border border-gray-700 text-left">
                  <p className="text-gray-500 text-[10px] font-bold uppercase mb-1">ID Acceso</p>
                  <p className="text-white font-mono text-lg">{staffSeleccionado.usuario}</p>
                </div>
                <div className="bg-[#0f172a] p-4 rounded-2xl border border-gray-700 text-left">
                  <p className="text-gray-500 text-[10px] font-bold uppercase mb-1">Estatus</p>
                  <p className={`font-black uppercase ${staffSeleccionado.estado === 'activo' ? 'text-green-400' : 'text-red-400'}`}>
